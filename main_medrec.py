...
parser.add_argument("--resume_from_checkpoint",
                    ...
                    type=str,
                    help="从指定检查点恢复训练")
parser.add_argument("--hierarchical_attention",
                    default=False,
                    action="store_true",
                    help="是否使用层级注意力校准机制")
parser.add_argument("--hier_consistency_weight",
                    default=0.1,
                    type=float,
                    help="层级一致性损失权重")
parser.add_argument("--med_voc_l1_size",
                    default=14,
                    type=int,
                    help="ATC1级别词汇表大小")
parser.add_argument("--med_voc_l2_size",
                    default=94,
                    type=int,
                    help="ATC2级别词汇表大小")
parser.add_argument("--med_voc_l3_size",
                    default=267,
                    type=int,
                    help="ATC3级别词汇表大小")
...
def main():
    ...
    if not args.Test:
        res, best_epoch, train_time = trainer.train(resume_from_checkpoint=args.resume_from_checkpoint)
        if args.log:
            log_res(args, res)
