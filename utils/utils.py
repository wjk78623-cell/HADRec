...
def ddi_rate_score(record, ddi_A):
    ...
    if len(record) > 0:
        print(f"\n🔍 ddi_rate_score 内部调试:")
        print(f"  record 长度: {len(record)}")
        print(f"  record[0] 类型: {type(record[0])}, 值: {record[0]}")
        if len(record[0]) > 0:
            print(f"  record[0][0] 类型: {type(record[0][0])}, 值: {record[0][0][:5] if hasattr(record[0][0], '__len__') and len(record[0][0]) > 5 else record[0][0]}")
    ...
    print(f"  all_cnt: {all_cnt}, dd_cnt: {dd_cnt}")
    if all_cnt == 0:
        return 0
