...
class Trainer(object):
    ...
    def train(self, resume_from_checkpoint=None):
        ...
        if resume_from_checkpoint and os.path.exists(resume_from_checkpoint):
            ...
            self.logger.info(f"从第 {start_epoch} 轮开始训练")

        for epoch in trange(start_epoch, int(self.args.num_train_epochs), desc="Epoch"):
            ...
            # 保存中间检查点
            if epoch % 5 == 0 or epoch == int(self.args.num_train_epochs) - 1:
                checkpoint_path = os.path.join(self.args.output_dir, f'checkpoint_epoch_{epoch}.pth')
                self._save_checkpoint(checkpoint_path, epoch, metric_dict[self.watch_metric])
    ...
    def _save_checkpoint(self, checkpoint_path, epoch, metric_value):
        """保存训练检查点"""
        checkpoint = {
            'epoch': epoch,
        ...
        self.logger.info(f"检查点已保存: {checkpoint_path}, 指标值: {metric_value:.4f}")
